...
int64_t ComputePipelineFlopBits(FunctionBase* f,
                                const PipelineSchedule& schedule) {
  int64_t total_flop_bits = 0;
  absl::flat_hash_set<Node*> dead_after_synthesis =
      GetDeadAfterSynthesisNodes(f);
  for (int64_t cycle = 0; cycle < schedule.length(); ++cycle) {
    for (Node* node : schedule.GetLiveOutOfCycle(cycle)) {
      if (dead_after_synthesis.contains(node)) {
        continue;
      }
      if (f->HasImplicitUse(node) ||
          absl::c_any_of(node->users(), [&](Node* user) {
            return schedule.IsScheduled(user) && schedule.cycle(user) > cycle &&
                   dead_after_synthesis.contains(user);
          })) {
        total_flop_bits += node->GetType()->GetFlatBitCount();
      }
    }
  }
  return total_flop_bits;
}

absl::StatusOr<int64_t> HybridScheduleScore(
    FunctionBase* f, const PipelineSchedule& schedule,
    const DelayEstimator& delay_estimator) {
  XLS_ASSIGN_OR_RETURN(
      int64_t max_stage_delay_ps,
      ComputeMaxStageDelayPs(f, schedule, delay_estimator));
  // Prefer fewer pipeline flop bits first, then a smaller maximum stage delay.
  return ComputePipelineFlopBits(f, schedule) * 1000000LL + max_stage_delay_ps;
}
...
absl::StatusOr<PipelineSchedule> RunPipelineScheduleInternal(
  ...
  if (options.strategy() == SchedulingStrategy::HYBRID) {
    SchedulingOptions sdc_options = options;
    sdc_options.strategy(SchedulingStrategy::SDC);
    XLS_ASSIGN_OR_RETURN(
        PipelineSchedule sdc_schedule,
        RunPipelineScheduleInternal(f, delay_estimator, sdc_options, elab,
                                    synthesizer));

    SchedulingOptions min_cut_options = options;
    min_cut_options.strategy(SchedulingStrategy::MIN_CUT);
    absl::StatusOr<PipelineSchedule> min_cut_schedule =
        RunPipelineScheduleInternal(f, delay_estimator, min_cut_options, elab,
                                    synthesizer);
    if (!min_cut_schedule.ok()) {
      VLOG(1) << "Hybrid scheduler falling back to SDC for `" << f->name()
              << "` because min-cut failed: " << min_cut_schedule.status();
      return sdc_schedule;
    }

    XLS_ASSIGN_OR_RETURN(int64_t sdc_score,
                         HybridScheduleScore(f, sdc_schedule, delay_estimator));
    XLS_ASSIGN_OR_RETURN(
        int64_t min_cut_score,
        HybridScheduleScore(f, *min_cut_schedule, delay_estimator));
    if (min_cut_score < sdc_score) {
      VLOG(1) << "Hybrid scheduler selected min-cut schedule for `"
              << f->name() << "` (score=" << min_cut_score
              << " vs sdc=" << sdc_score << ")";
      return *std::move(min_cut_schedule);
    }
    VLOG(1) << "Hybrid scheduler selected SDC schedule for `" << f->name()
            << "` (score=" << sdc_score << " vs min-cut=" << min_cut_score
            << ")";
    return sdc_schedule;
  }

  if (options.strategy() == SchedulingStrategy::HEURISTIC) {
